...
function wordsList(str, subStr) {
  ...
  let cleanStr = str
    .split('')
    .filter((item) => !separators.includes(item))
    .join('')
    .toLowerCase();
...
function getLocalDate(date, isSeconds = false, isISO = false) {
  ...
  let seconds = isSeconds ? ':' + addZero(date.getSeconds()) : '';
...
let getDateAgo = function (dt, days) {
  let date = new Date(dt);
  ...
  let changedDay = day < 10 ? '0' + day : day;
  let changedMonth = month < 10 ? '0' + month : month;

  return `${changedDay}.${changedMonth}.${year}`;
};
...
let car = new Car(1600, 'Corolla', 'Toyota', 2012);
let car2 = new Car(3500, 'IS 3', 'Lexus', 2020);
